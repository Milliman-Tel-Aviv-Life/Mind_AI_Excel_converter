...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    server: {
      ...
      strictPort: true,
      watch: { ignored: ['**/.figma/**'] },
      // Dev only: the FastAPI backend (excel-upload-preparation/app/web/server.py) serves /api.
      proxy: { '/api': { target: process.env.MIND_READY_API || 'http://localhost:8600', changeOrigin: true } },
    },
    preview: {
      host: '0.0.0.0',
      port: parseInt(process.env.PORT || '8443'),
