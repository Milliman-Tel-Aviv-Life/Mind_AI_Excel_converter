# Figma prompt -- "Mind Ready" interface for Excel Upload Preparation

Copy everything below the line into Figma (Figma Make / First Draft) or hand it to a designer. It describes the product exactly as it works today so the resulting front-end can be wired to the existing engine without inventing behaviour.
